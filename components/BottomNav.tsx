...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, onTabChange, requestCount = 0, onKaiPress }) => {
  // 5-item bottom nav: Today | Work | Kai (center orb) | Money | Profile
  const leftTabs: { id: TabType; label: string; icon: any }[] = [
  ...
  const renderTab = (tab: { id: TabType; label: string; icon: any }) => {
    const Icon = tab.icon
    ...
    return (
      <button
        ...
        style={{ minWidth: 48, padding: '8px 4px', flex: 1 }}
      >
      ...
      <div className="flex items-center justify-around h-16 max-w-lg mx-auto px-2">
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 1 }}>
          <button
            ...
            className="kai-orb-btn"
          >
            <div className="kai-orb">
              <span className="kai-orb-icon">✨</span>
            </div>
            <span className="kai-orb-label">Kai</span>
          </button>
        </div>

        {/* Right tabs: Money, Profile */}
